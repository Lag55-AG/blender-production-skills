...
def _mae(image: Image.Image) ->float:
    """Function _mae processes internal Blender data."""
...
def _crop_normalized(image: Image.Image, bbox: list[float]) ->Image.Image:
    """Function _crop_normalized processes internal Blender data."""
...
def compare(reference: Path, render: Path, output_dir: Path, tag: str,
    observations: (list[dict[str, object]] | None)=None) ->dict[str, object]:
    """Function compare processes internal Blender data."""
    with Image.open(reference) as ref_source, Image.open(render
        ) as render_source:
    ...
    aspect_error = abs(render_aspect - ref_aspect) / max(ref_aspect, 1e-09)
    ...
    for row in range(3):
        for column in range(4):
            ...
            grid_luminance_mae.append(_mae(ImageChops.difference(ref_region,
                result_region)))
    ...
    for observation in (observations or []):
        ...
        if not isinstance(bbox, list) or len(bbox) != 4:
            continue
        ref_region = _crop_normalized(ref_thumb, [float(value) for value in
            bbox])
        result_region = _crop_normalized(result_thumb, [float(value) for
            value in bbox])
        ref_region_gray = ImageOps.grayscale(ref_region)
        result_region_gray = ImageOps.grayscale(result_region)
        p0_metrics.append({'id': observation.get('id'), 'grayscale_mae':
            _mae(ImageChops.difference(ref_region_gray, result_region_gray)
            ), 'edge_mae': _mae(ImageChops.difference(ref_region_gray.
            filter(ImageFilter.FIND_EDGES), result_region_gray.filter(
    ...
    max_p0_luminance_mae = max((float(item['grayscale_mae']) for item in
        p0_metrics), default=0.0)
    max_p0_edge_mae = max((float(item['edge_mae']) for item in p0_metrics),
        default=0.0)
    if (aspect_error > 0.01 or grayscale_mae > 0.2 or
        max_grid_luminance_mae > 0.32 or max_p0_luminance_mae > 0.25 or
        max_p0_edge_mae > 0.3):
        status = 'FAIL'
    elif grayscale_mae > 0.12 or edge_mae > 0.18 or max_grid_luminance_mae > 0.18 or max_p0_luminance_mae > 0.15 or max_p0_edge_mae > 0.2:
        status = 'WARN'
    if not p0_metrics and status == 'PASS':
        status = 'WARN'
    return {'schema_version': '1.0', 'status': status, 'reference': str(
        reference), 'render': str(render), 'thumbnail_size': list(size),
        'aspect_ratio_error': aspect_error, 'grayscale_mae': grayscale_mae,
        'color_mae': color_mae, 'edge_mae': edge_mae, 'grid_luminance_mae':
        grid_luminance_mae, 'max_grid_luminance_mae':
        max_grid_luminance_mae, 'p0_metrics': p0_metrics, 'semantic_status':
        'REVIEW_REQUIRED' if p0_metrics else 'P0_OBSERVATIONS_MISSING',
        'overlay': str(overlay_path), 'difference': str(difference_path),
...
def main() ->int:
    ...
    observations = None
    if args.observation:
        observations = json.loads(Path(args.observation).expanduser().
            resolve().read_text(encoding='utf-8')).get('observations', [])
    report = compare(Path(args.reference).expanduser().resolve(), Path(args
        .render).expanduser().resolve(), Path(args.output_dir).expanduser()
        .resolve(), args.tag, observations)
