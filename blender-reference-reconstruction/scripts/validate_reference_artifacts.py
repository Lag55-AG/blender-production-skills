...
SCHEMA_VERSION = '1.0'
FILES = {'reference_derivatives.json', 'reference_observation.json',
    'spatial_hypothesis.json', 'camera_match.json',
    'material_hypotheses.json', 'visual_targets.json', 'reference_gate.json'}
REQUIRED_CHECKS = {'preflight': {'p0_observation_completeness',
    'spatial_hypothesis_completeness', 'camera_evidence',
    'blocking_uncertainty'}, 'blockout': {'camera_overlay',
    'negative_space_and_occlusion', 'portal_and_region_connectivity',
    'camera_spatial_consistency', 'cross_view_blockout',
    'directional_structure_skeleton', 'blockout_similarity_score'},
    'primary_surface': {'p0_primary_geometry', 'part_graph_complete',
    'blockout_replaced_by_formal_topology', 'structural_forms',
    'transition_forms', 'assembly_interfaces', 'real_bevel_geometry',
    'wireframe_acceptance', 'continuous_connections_and_smoothing',
    'semantic_material_identity'}, 'lookdev': {'causal_aging_and_wetness',
    'lighting_and_depth'}, 'final': {'final_overlay', 'final_semantic_review'}}
STAGE_ORDER = ['preflight', 'blockout', 'primary_surface', 'lookdev', 'final']
PRODUCTION_DOMAINS = {'geometry', 'material', 'lighting', 'atmosphere',
    'post', 'presentation', 'spatial_region'}
SPATIAL_CONNECTION_TYPES = {'opens_into', 'connected_by_stairs',
    'connected_by_ramp', 'connected_by_platform', 'corridor_continuation',
    'contains', 'adjacent', 'separated_by_boundary', 'support_contact',
    'continues_beyond_frame'}
SPATIAL_LAYERS = {'camera_enclosure', 'extreme_foreground', 'foreground',
    'midground', 'background', 'occluded', 'off_frame'}


def _bbox_valid(value: Any) ->bool:
    """Function _bbox_valid processes internal Blender data."""
    return isinstance(value, list) and len(value) == 4 and all(isinstance(
        item, (int, float)) for item in value) and all(0.0 <= float(item) <=
        1.0 for item in value) and float(value[2]) > 0.0 and float(value[3]
        ) > 0.0


def _confidence_valid(value: Any) ->bool:
    """Function _confidence_valid processes internal Blender data."""
    return isinstance(value, (int, float)) and 0.0 <= float(value) <= 1.0


def _unresolved(value: Any) ->bool:
    """Function _unresolved processes internal Blender data."""
    return value is None or isinstance(value, str) and value in {'',
        'unresolved'}


def _validate_derivatives(payload: dict[str, Any], require_resolved: bool
    ) ->tuple[list[str], list[str]]:
    """Function _validate_derivatives processes internal Blender data."""
    errors: list[str] = []
    warnings: list[str] = []
    ...
    if payload.get('generation_capability') not in {'unknown', 'available',
        'unavailable'}:
        errors.append('reference_derivatives_capability_invalid')
    allowed = {'pending', 'generated', 'skipped_capability_unavailable',
        'failed_non_blocking', 'skipped_no_reference'}
    for key in ('depth_map', 'white_model_guide'):
        ...
        if status == 'generated':
        ...
        elif status in {'skipped_capability_unavailable',
            ...
            if require_resolved:
                ...
                warnings.append(
                    f'reference_derivative_pending_non_blocking:{key}')
    return errors, warnings


def _validate_directional_structures(payload: dict[str, Any], stage: str,
    connections: list[dict[str, Any]]) ->list[str]:
    """Function _validate_directional_structures processes internal Blender data."""
    errors: list[str] = []
    structures = [item for item in payload.get('directional_structures', []
        ) if isinstance(item, dict)]
    required_by_connection = any(item.get('type') in {'connected_by_stairs',
        'connected_by_ramp'} for item in connections)
    if stage in {'blockout', 'primary_surface', 'lookdev', 'final'
        } and required_by_connection and not structures:
        errors.append('directional_structure_missing_for_elevation_connection')
    valid_types = {'stair_flight', 'stair_system', 'railing', 'handrail',
        'ramp', 'escalator', 'path_structure'}
    seen: set[str] = set()
    for item in structures:
        ...
        if not (isinstance(direction, list) and len(direction) == 3 and all
            (isinstance(value, (int, float)) for value in direction) and
            sum(float(value) ** 2 for value in direction) > 1e-12):
        ...
        points_valid = isinstance(control_path, list) and len(control_path
            ) >= 2 and all(isinstance(point, list) and len(point) == 3 and
            all(isinstance(value, (int, float)) for value in point) for
            point in control_path)
        if not points_valid:
            errors.append(f'directional_control_path:{structure_id}')
        elif isinstance(direction, list) and len(direction) == 3:
            path_vector = [(float(control_path[-1][axis]) - float(
                control_path[0][axis])) for axis in range(3)]
            if sum(path_vector[axis] * float(direction[axis]) for axis in
                range(3)) <= 0:
        ...
        if stage in {'blockout', 'primary_surface', 'lookdev', 'final'}:
            ...
            if not (isinstance(anchor_objects, dict) and isinstance(
                anchor_objects.get('start'), str) and anchor_objects.get(
                'start') and isinstance(anchor_objects.get('end'), str) and
                anchor_objects.get('end')):
                errors.append(f'directional_anchor_objects:{structure_id}')
            if not item.get('control_object_names'):
                errors.append(
                    f'directional_control_objects_empty:{structure_id}')
            if not item.get('generated_object_names'):
                errors.append(
                    f'directional_generated_objects_empty:{structure_id}')
        if structure_type in {'stair_flight', 'stair_system', 'escalator'}:
            if not isinstance(item.get('step_count'), int) or int(item.get(
                'step_count', 0)) < 1:
                errors.append(f'directional_step_count:{structure_id}')
            if not isinstance(item.get('rise'), (int, float)) or float(item
                .get('rise', 0)) <= 0:
                errors.append(f'directional_rise:{structure_id}')
            if not isinstance(item.get('run'), (int, float)) or float(item.
                get('run', 0)) <= 0:
            ...
            if points_valid:
                ...
                axis_index = {'X': 0, 'Y': 1, 'Z': 2}.get(axis_name.replace
                    ('-', ''), 2)
                axis_sign = -1.0 if axis_name.startswith('-') else 1.0
                elevation_delta = (float(control_path[-1][axis_index]) -
                    float(control_path[0][axis_index])) * axis_sign
                if item.get('ascending_from') == item.get('start_anchor'
                    ) and elevation_delta <= 0:
                    errors.append(f'directional_ascent_mismatch:{structure_id}'
                        )
                if item.get('ascending_from') == item.get('end_anchor'
                    ) and elevation_delta >= 0:
                    errors.append(f'directional_ascent_mismatch:{structure_id}'
                        )
        if structure_type in {'railing', 'handrail'}:
            ...
            if not isinstance(item.get('post_spacing'), (int, float)) or float(
                item.get('post_spacing', 0)) <= 0:
                errors.append(f'directional_post_spacing:{structure_id}')
            if points_valid and len(control_path) > 2 and _unresolved(item.
                get('bend_side')):
...
def _validate_blockout_state(gate: dict[str, Any], stage: str) ->list[str]:
    """Function _validate_blockout_state processes internal Blender data."""
    errors: list[str] = []
    ...
    history = [item for item in scoring.get('history', []) if isinstance(
        item, dict)]
    ...
    if current_score is not None and not (isinstance(current_score, (int,
        float)) and 0 <= float(current_score) <= 100):
        errors.append('blockout_score_out_of_range')
    disposition = scoring.get('disposition')
    if disposition not in {'not_scored', 'continue', 'rebuild_required',
    ...
    if project_status not in {'active', 'rebuild_required',
        'awaiting_deletion_decision', 'deletion_rejected', 'deletion_approved'
        }:
    ...
    if project_status == 'deletion_approved':
        ...
        if not (isinstance(confirmation, dict) and confirmation.get(
            'decision') == 'delete' and confirmation.get('confirmed_by') ==
            'user' and confirmation.get('confirmed_paths') == project.get(
            'deletion_candidate_paths')):
            errors.append('project_deletion_approval_invalid')
    return errors


def _validate_spatial_hypothesis(payload: dict[str, Any], stage: str) ->tuple[
    list[str], list[str]]:
    """Function _validate_spatial_hypothesis processes internal Blender data."""
    errors: list[str] = []
    warnings: list[str] = []
    ...
    if scene_kind not in {'interior', 'exterior', 'transitional',
        'object_stage', 'mixed'}:
        errors.append('spatial_scene_kind_unresolved')
    environment_like = scope == 'environment' or scene_kind in {'interior',
        'exterior', 'transitional', 'mixed'}
    ...
    if environment_like and not isinstance(frame.get('ground_elevation'), (
        int, float)):
        errors.append('coordinate_ground_elevation_missing')
    if not frame.get('evidence'):
        errors.append('coordinate_frame_evidence_empty')
    regions = [item for item in payload.get('regions', []) if isinstance(
        item, dict)]
    ...
    for region in regions:
        ...
        if region.get('visibility') not in {'observed', 'inferred',
            'required_support', 'off_frame_deferred'}:
            errors.append(f'spatial_region_visibility:{region_id}')
        if region.get('completion_tier') not in {'hero', 'support',
            'structural_stub', 'deferred'}:
    ...
    if camera_context.get('inside_outside') not in {'inside', 'outside',
        'threshold', 'object_stage'}:
    ...
    if environment_like and not any(item.get('role') == 'depth' for item in
        axes):
    ...
    connections = [item for item in payload.get('connections', []) if
        isinstance(item, dict)]
    adjacency: dict[str, set[str]] = {str(region_id): set() for region_id in
        region_ids}
    depth_connections = {'opens_into', 'connected_by_stairs',
        'connected_by_ramp', 'connected_by_platform',
        'corridor_continuation', 'continues_beyond_frame'}
    for connection in connections:
        ...
        if relation in depth_connections and connection.get('depth_required'
            ) is not True:
    ...
    if environment_like and camera_region in adjacency:
        ...
        for region in regions:
            ...
            if region.get('completion_tier') != 'deferred' and region.get(
                'visibility'
                ) != 'off_frame_deferred' and region_id not in reachable:
    ...
    anchors = [item for item in payload.get('scale_anchors', []) if
        isinstance(item, dict)]
    if not anchors:
    ...
    for anchor in anchors:
        ...
        if not (isinstance(value_range, list) and len(value_range) == 2 and
            all(isinstance(value, (int, float)) and float(value) > 0.0 for
            value in value_range) and float(value_range[1]) >= float(
            value_range[0])):
    ...
    blockers = [item for item in payload.get('alternative_hypotheses', []) if
        isinstance(item, dict) and item.get('impact') == 'blocking' and
        item.get('status') not in {'resolved', 'accepted_variant'}]
    if blockers:
    ...
    if stage in {'blockout', 'primary_surface', 'lookdev', 'final'}:
        ...
        views = [item for item in payload.get('blockout_views', []) if
            isinstance(item, dict)]
        passed_types = {item.get('type') for item in views if item.get(
            'status') == 'passed' and Path(str(item.get('path', ''))).is_file()
            }
        required_types = {'camera', 'top', 'front', 'side'
            } if environment_like else {'camera'}
        ...
        if not environment_like and not passed_types.intersection({'top',
            'front', 'side'}):
            warnings.append('structural_review_view_missing')
    return errors, warnings


def validate(directory: Path, stage: str, construction_graph: (dict[str,
    Any] | None)=None) ->dict[str, Any]:
    """Function validate processes internal Blender data."""
    errors: list[str] = []
    warnings: list[str] = []
    ...
    if errors:
        return {'schema_version': SCHEMA_VERSION, 'status': 'FAIL', 'stage':
            stage, 'errors': errors, 'warnings': warnings}
    derivative_errors, derivative_warnings = _validate_derivatives(payloads
        ['reference_derivatives.json'], require_resolved=stage !=
        'blockout_entry')
    ...
    for source in sources:
        ...
        if int(source.get('width', 0)) <= 0 or int(source.get('height', 0)
            ) <= 0:
    ...
    if stage == 'blockout_entry':
        ...
        return {'schema_version': SCHEMA_VERSION, 'status': 'FAIL' if
            errors else 'WARN' if warnings else 'PASS', 'stage': stage,
    ...
    blockers = [item for item in observation.get('uncertainty_register', []
        ) if item.get('impact') == 'blocking' and item.get('status') not in
        {'resolved', 'accepted_variant'}]
    if blockers:
        errors.append('unresolved_blocking_uncertainty')
    spatial = payloads['spatial_hypothesis.json']
    spatial_errors, spatial_warnings = _validate_spatial_hypothesis(spatial,
        stage)
    ...
    if stage in {'blockout', 'primary_surface', 'lookdev', 'final'}:
        ...
        if camera.get('revision') != spatial.get('camera_context', {}).get(
            'revision'):
    ...
    for item in p0:
        if item.get('requires_material') is True and item.get('id'
            ) not in material_entities:
    ...
    for item in materials:
        ...
        if stage in {'primary_surface', 'lookdev', 'final'}:
            if not item.get('selected_hypothesis'):
                errors.append(
                    f"material_selection_missing:{item.get('entity_id')}")
            if not item.get('validation_requirements'):
                errors.append(
                    f"material_validation_requirements_missing:{item.get('entity_id')}"
                    )
    if stage in {'primary_surface', 'lookdev', 'final'}:
        if not construction_graph:
            ...
            graph_parts = {item.get('id'): item for item in
                construction_graph.get('parts', []) if isinstance(item,
                dict) and item.get('id')}
            spatial_region_ids = {item.get('id') for item in spatial.get(
                'regions', []) if isinstance(item, dict) and item.get('id')}
            for item in p0:
                item_id = item.get('id')
                ...
                if domain == 'geometry':
                    ...
                    if part is None:
                        ...
                        errors.append(
                            f'p0_construction_requirements_empty:{item_id}')
                    else:
                        if part.get('blockout_proxy') is not False:
                            errors.append(
                                f'p0_blockout_proxy_remains:{item_id}')
                        if part.get('topology_status') != 'passed':
                            errors.append(
                                f'p0_formal_topology_not_passed:{item_id}')
                        if part.get('final_object_name') in {None, '',
                            'unresolved'}:
                        ...
                        if not topology_evidence.get('construction_operations'
                            ...
                            errors.append(
                                f'p0_construction_operations_empty:{item_id}')
                        component_count = topology_evidence.get(
                            'connected_component_count')
                        if not isinstance(component_count, int
                            ) or component_count < 1:
                            errors.append(
                                f'p0_component_evidence_missing:{item_id}')
                        wireframe = topology_evidence.get('wireframe')
                        if not isinstance(wireframe, str) or not Path(wireframe
                            ).is_file():
                            errors.append(
                                f'p0_wireframe_evidence_missing:{item_id}')
                        bevel_policy = part.get('bevel_policy', {})
                        if bevel_policy.get('method') in {None, '',
                            'unresolved'}:
            ...
            for index, relation in enumerate(construction_graph.get(
                'relationships', [])):
                if not isinstance(relation, dict) or not relation.get(
                    'validation'):
                    errors.append(
                        f'construction_relationship_validation_empty:{index}')
            if construction_graph.get(
                'unclassified_visible_intersections_allowed') is not False:
                errors.append(
                    'unclassified_visible_intersections_not_forbidden')
    visual = payloads['visual_targets.json']
    for key in ('attention_order', 'luminance_zones', 'light_sources',
        'depth_layers'):
    ...
    required_gate = {'preflight': 'R0', 'blockout': 'R1', 'primary_surface':
        'R2', 'lookdev': 'R3', 'final': 'R4'}[stage]
    if gate_order.get(gate.get('current_gate'), -1) < gate_order[required_gate
        ]:
        errors.append(
            f"gate_below_stage:{gate.get('current_gate')}<{required_gate}")
    ...
    if stage in {'blockout', 'primary_surface', 'lookdev', 'final'
        } and not gate.get('evidence_paths'):
    ...
    gate_checks = {item.get('id'): item for item in gate.get('checks', []) if
    ...
    return {'schema_version': SCHEMA_VERSION, 'status': 'FAIL' if errors else
        'WARN' if warnings else 'PASS', 'stage': stage, 'p0_count': len(p0),
        'material_count': len(materials), 'errors': sorted(set(errors)),
...
def main() ->int:
    ...
    parser.add_argument('--stage', choices=['blockout_entry', 'preflight',
        'blockout', 'primary_surface', 'lookdev', 'final'], default='preflight'
        )
    ...
    if args.construction_graph:
        graph = json.loads(Path(args.construction_graph).expanduser().
            resolve().read_text(encoding='utf-8'))
    report = validate(Path(args.artifact_dir).expanduser().resolve(), args.
        stage, graph)
