...
def validate(plan: dict[str, Any], require_passed: bool=False) ->dict[str, Any
    ...
    for index, light in enumerate(lights):
        ...
        for key in ('source', 'role', 'evidence', 'loss_if_removed'):
            value = light.get(key)
            if (value is None or value == '' or value == 'unresolved' or
                value == []):
    ...
    return {'schema_version': '1.0', 'status': 'FAIL' if errors else 'PASS',
        'light_count': len(lights), 'errors': sorted(set(errors))}
...
def main() ->int:
    ...
    plan = json.loads(Path(args.lighting_plan).expanduser().resolve().
