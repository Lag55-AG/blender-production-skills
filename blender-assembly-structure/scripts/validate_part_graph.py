...
STAGES = ['analysis', 'blockout', 'topology_construction',
    'structural_forms', 'transition_forms', 'functional_parts',
    'surface_details', 'systems', 'surfacing', 'lighting', 'final']


def validate(graph: dict[str, Any], require_approved: bool=False, stage:
    str='analysis') ->dict[str, Any]:
    ...
    for part in parts:
        ...
        if part.get('role') in BUILDABLE_ROLES:
            ...
            if part.get('separation_policy') == 'continuous_shell':
                ...
                if part.get('requirements', {}).get('single_component'
                    ) is not True:
                    errors.append(
                        f'continuous_shell_not_single_component:{part_id}')
            if stage_index > STAGES.index('topology_construction'
                ) and part.get('blockout_proxy') is True:
                errors.append(f'blockout_proxy_remains:{part_id}')
            if stage_index > STAGES.index('topology_construction'
                ) and part.get('form_level') in {'primary', 'structural',
                'transition'} and part.get('topology_status') not in {'passed',
    ...
    for index, relation in enumerate(graph.get('relationships', [])):
        ...
        endpoints = [part for part in parts if part.get('id') in {relation.
            get('a'), relation.get('b')}]
        if relation_type in {'continuous_surface', 'boolean_fused'} and any(
            part.get('combination_level') != 'D_TOPOLOGY_FUSION' for part in
            endpoints):
            errors.append(f'fusion_relation_not_level_d:{index}')
    return {'schema_version': '1.0', 'status': 'FAIL' if errors else 'PASS',
        'part_count': len(parts), 'errors': sorted(set(errors))}
...
def main() ->int:
    ...
    graph = json.loads(Path(args.construction_graph).expanduser().resolve()
