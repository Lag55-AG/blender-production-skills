...
def _box(name: str, dimensions: tuple[float, float, float], location: tuple
    [float, float, float]) ->bpy.types.Object:
    """Function _box processes internal Blender data."""
    hx, hy, hz = (value * 0.5 for value in dimensions)
    vertices = [(-hx, -hy, -hz), (hx, -hy, -hz), (hx, hy, -hz), (-hx, hy, -
        hz), (-hx, -hy, hz), (hx, -hy, hz), (hx, hy, hz), (-hx, hy, hz)]
...
def _evaluated_mesh_stats(obj: bpy.types.Object) ->dict[str, Any]:
    """Function _evaluated_mesh_stats processes internal Blender data."""
    ...
    try:
        ...
        return {'vertices': len(mesh.vertices), 'edges': len(mesh.edges),
            'faces': len(mesh.polygons), 'min': [min(co[index] for co in
            coordinates) for index in range(3)], 'max': [max(co[index] for
            co in coordinates) for index in range(3)]}
    finally:
        evaluated.to_mesh_clear()


def _test_architectural_opening() ->dict[str, Any]:
    """Function _test_architectural_opening processes internal Blender data."""
    ...
    opening_hit, *_ = evaluated.ray_cast(Vector((0.0, -2.0, -0.5)), Vector(
        (0.0, 1.0, 0.0)))
    solid_hit, *_ = evaluated.ray_cast(Vector((2.0, -2.0, -0.5)), Vector((
        0.0, 1.0, 0.0)))
    ...
    return {'host': wall.name, 'cutter': cutter.name, 'modifier': modifier.
        type, 'operation': modifier.operation, 'solver': modifier.solver,
        'opening_ray_clear': not opening_hit, 'solid_wall_ray_hits':
        solid_hit, 'evaluated': stats}


def _test_stair_array() ->dict[str, Any]:
    """Function _test_stair_array processes internal Blender data."""
    step = _box('STAIR-STEP-SOURCE', (1.2, 0.32, 0.18), (0.0, 0.0, 0.09))
    modifier = step.modifiers.new('ARRAY-STAIR-FLIGHT', 'ARRAY')
    modifier.count = 7
    modifier.use_relative_offset = False
    modifier.use_constant_offset = True
    modifier.constant_offset_displace = 0.0, 0.3, 0.18
    bpy.context.view_layer.update()
    stats = _evaluated_mesh_stats(step)
    y_extent = stats['max'][1] - stats['min'][1]
    z_extent = stats['max'][2] - stats['min'][2]
    expected_y = 0.32 + 0.3 * (modifier.count - 1)
    expected_z = 0.18 + 0.18 * (modifier.count - 1)
    if not math.isclose(y_extent, expected_y, abs_tol=1e-05):
        raise RuntimeError(
            f'Unexpected stair run extent: {y_extent} != {expected_y}')
    if not math.isclose(z_extent, expected_z, abs_tol=1e-05):
        raise RuntimeError(
            f'Unexpected stair rise extent: {z_extent} != {expected_z}')
    return {'source': step.name, 'modifier': modifier.type, 'count':
        modifier.count, 'constant_offset': list(modifier.
        constant_offset_displace), 'run_extent': y_extent, 'rise_extent':
        z_extent, 'evaluated': stats}


def _test_curve_profile() ->dict[str, Any]:
    """Function _test_curve_profile processes internal Blender data."""
    ...
    for point, coordinate in zip(profile_spline.points, [(-0.05, -0.035,
        0.0, 1.0), (0.05, -0.035, 0.0, 1.0), (0.05, 0.035, 0.0, 1.0), (-
        0.05, 0.035, 0.0, 1.0)]):
        point.co = coordinate
    ...
    coordinates = [(0.0, 0.0, 0.0), (1.0, 0.3, 0.2), (1.8, 1.2, 0.5), (2.2,
        2.2, 0.8)]
    ...
    if stats['vertices'] <= 8 or stats['faces'] <= 4:
        raise RuntimeError(
            f'Bezier profile did not evaluate to usable mesh geometry: {stats}'
            )
    if path_data.bevel_object != profile or spline.type != 'BEZIER':
        raise RuntimeError('Curve profile ownership was not preserved')
    return {'path': path.name, 'path_type': spline.type, 'control_points':
        len(spline.bezier_points), 'profile': profile.name, 'bevel_mode':
        path_data.bevel_mode, 'evaluated': stats}


def _test_precision_face_placement() ->dict[str, Any]:
    """Function _test_precision_face_placement processes internal Blender data."""
    target = _box('SNAP-TARGET', (2.0, 2.0, 0.2), (0.0, 0.0, 0.0))
    source = _box('SNAP-SOURCE', (0.5, 0.5, 0.1), (0.0, 0.0, 2.0))
    tool_settings = bpy.context.scene.tool_settings
    snap_enum = {item.identifier for item in tool_settings.bl_rna.
        properties['snap_elements'].enum_items}
    ...
    hit, location, normal, _index = target.ray_cast(Vector((0.0, 0.0, 2.0)),
        Vector((0.0, 0.0, -1.0)))
    if not hit:
    ...
    if not math.isclose(measured_gap, offset, abs_tol=1e-06):
        raise RuntimeError(
            f'Snapped placement gap mismatch: {measured_gap} != {offset}')
    return {'source': source.name, 'target': target.name, 'snap_elements':
        sorted(tool_settings.snap_elements), 'align_rotation': getattr(
        tool_settings, 'use_snap_align_rotation', None), 'measured_gap':
        measured_gap, 'target_normal': list(world_normal)}


def _grid(name: str, count: int, spacing: float) ->bpy.types.Object:
    """Function _grid processes internal Blender data."""
    half = (count - 1) * spacing * 0.5
    vertices = [(x * spacing - half, y * spacing - half, 0.0) for y in
        range(count) for x in range(count)]
    faces = []
...
def _test_displace_surface() ->dict[str, Any]:
    """Function _test_displace_surface processes internal Blender data."""
    ...
    if stats['vertices'] <= len(surface.data.vertices) or z_range <= 0.01:
        raise RuntimeError(
            f'Displace did not create evaluated geometric relief: {stats}')
    return {'surface': surface.name, 'stack': stack, 'source_vertices': len
        (surface.data.vertices), 'evaluated_vertices': stats['vertices'],
        'texture': texture.name, 'texture_coords': modifier.texture_coords,
        'coordinate_object': coordinate_object.name, 'direction': modifier.
        direction, 'midlevel': modifier.mid_level, 'strength': modifier.
        strength, 'z_range': z_range}


def _run(name: str, fn: Callable[[], dict[str, Any]]) ->dict[str, Any]:
    """Function _run processes internal Blender data."""
    try:
...
def main() ->int:
    ...
    tests: list[tuple[str, Callable[[], dict[str, Any]]]] = [(
        ...
        'stair_array_rise_run', _test_stair_array), ('bezier_curve_profile',
        _test_curve_profile), ('precision_face_placement',
        _test_precision_face_placement), ('subdivision_displace',
        _test_displace_surface)]
    ...
    report = {'schema_version': '1.0', 'generated_at': dt.datetime.now(dt.
        ...
        version_string, 'background': bpy.app.background,
        'factory_startup_fixture': True, 'status': 'PASS' if not failed else
        'FAIL', 'passed': len(results) - len(failed), 'failed': len(failed),
        'results': results}
    output = Path(args.output).expanduser().resolve()
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(report, indent=2, ensure_ascii=False),
        encoding='utf-8')
    print('CONSTRUCTION_METHOD_INTEGRATION ' + json.dumps(report,
        ensure_ascii=False))
