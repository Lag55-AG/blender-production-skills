...
def _require_runtime() ->None:
    """Function _require_runtime processes internal Blender data."""
    ...
    if not hasattr(bpy.types, 'ShaderNodeRaycast'):
        raise RuntimeError(
            'ShaderNodeRaycast is unavailable in this Blender runtime')
...
def _claim_shader_group(preferred_name: str, role: str) ->bpy.types.NodeTree:
    """Function _claim_shader_group processes internal Blender data."""
    for group in bpy.data.node_groups:
        if group.bl_idname == 'ShaderNodeTree' and group.get(OWNER_PROP
            ) == OWNER and group.get(ROLE_PROP) == role:
            return group
    group = bpy.data.node_groups.get(preferred_name)
    if group is not None and (group.bl_idname != 'ShaderNodeTree' or group.
        get(OWNER_PROP) != OWNER):
...
def _locate(node: bpy.types.Node, x: float, y: float) ->bpy.types.Node:
    """Function _locate processes internal Blender data."""
    node.location = x, y
    return node


def build_raycast_sample_group(*, width: float=0.02, ray_length: float=
    100.0, only_local: bool=True) ->bpy.types.NodeTree:
    ...
    _new_socket(group, 'Offset', 'INPUT', 'NodeSocketVector', default=(1.0,
        0.0, 0.0))
    _new_socket(group, 'Width', 'INPUT', 'NodeSocketFloat', default=width,
        minimum=0.0, maximum=max(width * 20.0, 1.0))
    _new_socket(group, 'Ray Length', 'INPUT', 'NodeSocketFloat', default=
        ray_length, minimum=0.001, maximum=max(ray_length * 100.0, 1000.0))
...
def build_look_group(sample_group: bpy.types.NodeTree, *, base_color:
    Sequence[float]=(0.42, 0.62, 0.82, 1.0), outline_color: Sequence[float]
    =(0.004, 0.006, 0.01, 1.0), width: float=0.02, ray_length: float=100.0,
    toon_size: float=0.52, toon_smooth: float=0.03) ->bpy.types.NodeTree:
    """Build the four-direction mask and Cycles-compatible toon look."""
    group = _claim_shader_group(LOOK_GROUP_NAME, 'raycast_look')
    group.nodes.clear()
    _clear_interface(group)
    _new_socket(group, 'Base Color', 'INPUT', 'NodeSocketColor', default=
        tuple(base_color))
    _new_socket(group, 'Outline Color', 'INPUT', 'NodeSocketColor', default
        =tuple(outline_color))
    _new_socket(group, 'Width', 'INPUT', 'NodeSocketFloat', default=width,
        minimum=0.0, maximum=max(width * 20.0, 1.0))
    _new_socket(group, 'Ray Length', 'INPUT', 'NodeSocketFloat', default=
        ray_length, minimum=0.001, maximum=max(ray_length * 100.0, 1000.0))
    _new_socket(group, 'Toon Size', 'INPUT', 'NodeSocketFloat', default=
        toon_size, minimum=0.0, maximum=1.0)
    _new_socket(group, 'Toon Smooth', 'INPUT', 'NodeSocketFloat', default=
        toon_smooth, minimum=0.0, maximum=1.0)
    ...
    offsets = [('BPS Ray +X', (1.0, 0.0, 0.0), -700, 310), ('BPS Ray -X', (
        -1.0, 0.0, 0.0), -700, 120), ('BPS Ray +Y', (0.0, 1.0, 0.0), -700,
        -70), ('BPS Ray -Y', (0.0, -1.0, 0.0), -700, -260)]
    ...
    for name, offset, x, y in offsets:
        ...
        links.new(group_input.outputs['Ray Length'], sample.inputs[
            'Ray Length'])
...
def build_material(look_group: bpy.types.NodeTree) ->bpy.types.Material:
    """Function build_material processes internal Blender data."""
...
def apply_to_objects(objects: Iterable[bpy.types.Object], *, width: float=
    0.02, ray_length: float=100.0, only_local: bool=True, base_color:
    Sequence[float]=(0.42, 0.62, 0.82, 1.0), outline_color: Sequence[float]
    =(0.004, 0.006, 0.01, 1.0), toon_size: float=0.52, toon_smooth: float=
    0.03, replace_material: bool=False) ->dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError('Cycles is unavailable in this Blender runtime'
            ) from exc
    sample_group = build_raycast_sample_group(width=width, ray_length=
        ray_length, only_local=only_local)
    look_group = build_look_group(sample_group, base_color=base_color,
        outline_color=outline_color, width=width, ray_length=ray_length,
        toon_size=toon_size, toon_smooth=toon_smooth)
    material = build_material(look_group)
    results = []
    for obj in mesh_objects:
        action = _assign_material(obj, material, replace_material=
            replace_material)
        results.append({'object': obj.name, 'material_action': action})
    return {'schema_version': SCHEMA_VERSION, 'owner': OWNER,
        'blender_version': bpy.app.version_string, 'render_engine': bpy.
        context.scene.render.engine, 'objects': results, 'sample_group':
        sample_group.name, 'look_group': look_group.name, 'material':
        material.name, 'ray_samples': 4, 'offsets': [[1, 0, 0], [-1, 0, 0],
        [0, 1, 0], [0, -1, 0]], 'width': width, 'ray_length': ray_length,
        'only_local': only_local, 'replace_material': replace_material}
...
def main() ->int:
    ...
    parser.add_argument('--base-color', type=float, nargs=3, default=(0.42,
        0.62, 0.82))
    ...
    report = apply_to_objects(objects, width=args.width, ray_length=args.
        ray_length, only_local=not args.scene_aware, base_color=(*args.
        base_color, 1.0), outline_color=(*args.outline_color, 1.0),
        toon_size=args.toon_size, toon_smooth=args.toon_smooth,
