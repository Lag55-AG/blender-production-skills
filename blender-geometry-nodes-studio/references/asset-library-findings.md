...
These observations came from a read-only study of `F:\Work Study\CG Design\blender\Node Assets`: 14 Blender files, 32 Geometry Nodes groups, 5,953 nodes, and 7,372 links. Connection coordinates were measured after resolving nested Frame offsets.
...
`Grass.blend` keeps individual grass types in separate 78-186 node groups, then uses a compact `Grass` assembly group to combine four sub-groups. `Data Flow.blend` splits generation, sampling, calculation, transfer, curvature sampling, and port extraction into nine groups; its `Data Flow` assembly group connects seven functional groups.
