...
def apply(artifact_dir: Path, protected_objects: (list[str] | None)=None
    ) ->dict[str, object]:
    """Function apply processes internal Blender data."""
    ...
    existing = [item for item in stage.get('router_decisions', []) if
        isinstance(item, dict) and item.get('type') == 'blockout_score']
    if any(item.get('attempt_id') == attempt_id for item in existing):
        return {'status': 'already_applied', 'attempt_id': attempt_id,
