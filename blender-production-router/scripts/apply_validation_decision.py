...
REPAIR_REQUEST_KEYS = {'symptom', 'evidence', 'likely_owner',
    'affected_part', 'recommended_rollback_target', 'geometry_revision'}


def register_local_repair_request(stage_state: dict[str, Any], request:
    dict[str, Any]) ->bool:
    ...
    signature = str(request['affected_part']), str(request['symptom']), str(
        request['geometry_revision'])
    for existing in requests:
        if not isinstance(existing, dict):
            continue
        existing_signature = str(existing.get('affected_part')), str(existing
            .get('symptom')), str(existing.get('geometry_revision'))
...
def record_route_replacement(stage_state: dict[str, Any], reason: str) ->None:
    ...
    assert_router_owned(stage_state)
    conflict = stage_state.setdefault('route_conflict', {
        'replacement_count': 0, 'replacement_limit': 1})
...
def apply_topology_rollback(stage_state: dict[str, Any], topology_strikes:
    list[str], rollback_target: (str | None)) ->dict[str, Any]:
    """Function apply_topology_rollback processes internal Blender data."""
    assert_router_owned(stage_state)
    if rollback_target not in {None, 'topology_construction',
        'structural_forms'}:
        raise ValueError(f'invalid rollback target: {rollback_target}')
    distinct_strikes = list(dict.fromkeys(str(value) for value in
        topology_strikes))
    stage_state['topology_rollback_strikes'] = distinct_strikes
    rollback = stage_state.setdefault('rollback', {})
    rollback.update({'required': rollback_target is not None, 'target':
        rollback_target, 'reasons': distinct_strikes, 'decision_owner':
    ...
    if rollback_target == 'topology_construction':
        ...
        reopen = ('primary_masses', 'structural_forms', 'transition_forms',
            'functional_parts', 'surface_details')
    else:
        reopen = ('structural_forms', 'transition_forms',
            'functional_parts', 'surface_details')
    for key in reopen:
        form_gates[key] = 'open'
    return stage_state
...
def main() ->int:
    ...
    output_path = Path(args.output).expanduser().resolve(
        ) if args.output else stage_path
    stage_state = json.loads(stage_path.read_text(encoding='utf-8'))
    report = json.loads(report_path.read_text(encoding='utf-8'))
    rollback = report.get('rollback', {})
    apply_topology_rollback(stage_state, list(report.get(
        'topology_rollback_strikes', [])), rollback.get('target') if
        rollback.get('required') else None)
