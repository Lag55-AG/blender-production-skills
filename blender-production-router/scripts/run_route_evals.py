...
from route_blender_task import classify_request


def main() ->int:
    """Function main processes internal Blender data."""
    parser = argparse.ArgumentParser()
    parser.add_argument('--evals', default=str(SCRIPT_DIR.parent / 'evals' /
        'route_evals.json'))
    ...
    for case in cases:
        ...
        candidate_methods = [candidate['method'] for candidate in route[
            'candidates']]
        archetypes = {item['id']: item for item in route.get(
            'construction_archetypes', [])}
        parameter_owner_text = ' '.join(owner for owners in route.get(
            'construction_method_decision', {}).get('parameter_owners', {})
            .values() for owner in owners).lower()
        errors = []
        ...
        required_choice_fields = {'comparison_required', 'direct_candidate',
        ...
        if missing_choice_fields:
            errors.append('missing system choice fields: ' + ', '.join(
                sorted(missing_choice_fields)))
        if route.get('code_role') not in {'orchestration',
            'direct_topology_exception', 'none'}:
            errors.append(f"invalid code role {route.get('code_role')}")
        if route.get('application_policy') not in {'keep_non_destructive',
            'apply_for_downstream_topology', 'realize_for_export',
            'bake_simulation', 'not_applicable'}:
            errors.append(
                f"invalid application policy {route.get('application_policy')}"
                )
        expected_selected = case.get('expected_selected')
        if expected_selected and route['selected_method'] != expected_selected:
            errors.append(
                f"selected {route['selected_method']} != {expected_selected}")
        expected_skill = case.get('expected_skill')
        if expected_skill and route['selected_skill'] != expected_skill:
            errors.append(
                f"skill {route['selected_skill']} != {expected_skill}")
        expected_deliverable = case.get('expected_deliverable')
        if expected_deliverable and route['deliverable'
            ] != expected_deliverable:
            errors.append(
                f"deliverable {route['deliverable']} != {expected_deliverable}"
                )
        expected_design_owner = case.get('expected_design_owner')
        if expected_design_owner and route.get('design_owner'
            ) != expected_design_owner:
            errors.append(
                f"design owner {route.get('design_owner')} != {expected_design_owner}"
                )
        expected_native_system = case.get('expected_native_system')
        if expected_native_system and native_decision.get('primary_system'
            ) != expected_native_system:
            errors.append(
                f"native system {native_decision.get('primary_system')} != {expected_native_system}"
                )
        if 'expected_node_candidate' in case and system_choice.get(
            'node_candidate') != case['expected_node_candidate']:
            errors.append(
                f"node candidate {system_choice.get('node_candidate')} != {case['expected_node_candidate']}"
                )
        expected_library_status = case.get(
            'expected_local_asset_library_status')
        if expected_library_status and route.get('local_asset_library', {}
            ).get('status') != expected_library_status:
            errors.append(
                f"local asset library status {route.get('local_asset_library', {}).get('status')} != {expected_library_status}"
                )
        expected_library_origin = case.get(
            'expected_local_asset_library_origin')
        if expected_library_origin and route.get('local_asset_library', {}
            ).get('request_origin') != expected_library_origin:
            errors.append(
                f"local asset library origin {route.get('local_asset_library', {}).get('request_origin')} != {expected_library_origin}"
                )
        ...
        for archetype_id, expected_method in case.get(
            'expected_archetype_methods', {}).items():
        ...
        results.append({'id': case['id'], 'request': case['request'],
            'selected': route['selected_method'], 'candidates':
            candidate_methods, 'status': 'PASS' if not errors else 'FAIL',
            'errors': errors})
    failed = [result for result in results if result['status'] == 'FAIL']
    report = {'schema_version': '1.0', 'status': 'PASS' if not failed else
        'FAIL', 'case_count': len(results), 'passed': len(results) - len(
        failed), 'failed': len(failed), 'results': results}
