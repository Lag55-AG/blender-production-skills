...
def read_json(path: Path) ->dict[str, Any]:
    """Function read_json processes internal Blender data."""
...
def write_json_atomic(path: Path, payload: dict[str, Any]) ->None:
    """Function write_json_atomic processes internal Blender data."""
...
def assert_router_owned(stage_state: dict[str, Any]) ->None:
    """Function assert_router_owned processes internal Blender data."""
    authority = stage_state.get('authority', {})
    if authority.get('state_owner') != ROUTER_OWNER:
        raise ValueError(
            'stage_state is not owned by blender-production-router')


def _set_membership(values: list[str], part_id: str, include: bool) ->list[str
    ]:
    """Function _set_membership processes internal Blender data."""
    unique = [value for value in dict.fromkeys(str(item) for item in values
        ) if value != part_id]
    if include:
...
def update_part_progress(stage_state: dict[str, Any], part_id: str,
    critical: bool, disposition: str, gate_clear: bool) ->None:
    """Apply a local part review without changing the route or deletion policy."""
    assert_router_owned(stage_state)
    progress = stage_state.setdefault('part_progress', {'active': [],
    ...
    for key in ('active', 'continuable', 'paused', 'needs_user_review'):
        if not isinstance(progress.get(key), list):
            progress[key] = []
    if gate_clear:
        progress['continuable'] = _set_membership(progress['continuable'],
            part_id, True)
        progress['paused'] = _set_membership(progress['paused'], part_id, False
            )
        progress['needs_user_review'] = _set_membership(progress[
            'needs_user_review'], part_id, False)
    else:
        ...
        progress['needs_user_review'] = _set_membership(progress[
            'needs_user_review'], part_id, disposition == 'needs_user_review')
    if disposition == 'needs_user_review' and critical and stage_state.get(
        'visual_gate') in {'blockout', 'primary_surface', 'systems', 'final'}:
        stage_state['gate_status'] = 'waiting_for_user'
        stage_state['mutations_blocked'] = True
        stage_state['allowed_operations'] = ['inspect evidence',
            'review the failed critical part', 'record the user decision']


def apply_blockout_score_decision(stage_state: dict[str, Any], gate: dict[
    str, Any], protected_objects: (list[str] | None)=None) ->dict[str, Any]:
    """Apply the latest reference score to Router state and return the decision."""
    assert_router_owned(stage_state)
    if gate.get('state_authority') != 'stage_state.json':
        raise ValueError(
            'reference_gate does not declare stage_state.json as authority')
    ...
    disposition = decision.get('project_disposition_patch', gate.get(
        'project_disposition', {}))
    if not isinstance(disposition, dict):
        raise ValueError(
            'reference_gate Router decision has an invalid project disposition patch'
            )
    ...
    stage_state['project_disposition'] = {**stage_state.get(
        'project_disposition', {}), **disposition}
    if protected_objects is not None:
        stage_state['protected_objects'] = list(dict.fromkeys(str(item) for
            item in protected_objects))
    ...
    stage_state.setdefault('router_decisions', []).append({'type':
        'blockout_score', 'attempt_id': attempt.get('attempt_id'), 'score':
        score, 'consecutive_under_40': consecutive, 'action': action})
    gate['project_disposition'] = dict(stage_state['project_disposition'])
    result: dict[str, Any] = {'action': action, 'attempt_id': attempt.get(
        'attempt_id'), 'score': score, 'consecutive_under_40': consecutive,
    ...
    if action == 'stop_and_request_deletion_decision':
        result['user_decision_required'] = {'question':
            ...
            'deletion_candidate_paths': stage_state['project_disposition'].
            get('deletion_candidate_paths', []), 'protected_objects':
            stage_state.get('protected_objects', [])}
    return result
