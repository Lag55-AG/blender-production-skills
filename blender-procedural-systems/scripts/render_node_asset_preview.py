...
def _look_at(obj: bpy.types.Object, target: Vector) ->None:
    """Function _look_at processes internal Blender data."""
    obj.rotation_euler = (target - obj.location).to_track_quat('-Z', 'Y'
        ).to_euler()
...
def main() ->int:
    ...
    points = [(obj.matrix_world @ Vector(corner)) for obj in meshes for
    ...
    camera.location = center + Vector((1.8, -2.6, 1.35)).normalized(
        ) * extent * 3.2
    camera_data.lens = 58
    ...
    _look_at(rim, center)
    world = bpy.context.scene.world or bpy.data.worlds.new(
        'NODE_ASSET_REVIEW_WORLD')
