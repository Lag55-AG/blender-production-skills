...
def _claim_material(preferred_name: str, role: str) ->bpy.types.Material:
    """Function _claim_material processes internal Blender data."""
    for material in bpy.data.materials:
        if material.get(OWNER_PROP) == OWNER and material.get(ROLE_PROP
            ) == role:
            return material
...
def _claim_node_group(preferred_name: str, role: str) ->bpy.types.NodeTree:
    """Function _claim_node_group processes internal Blender data."""
    for group in bpy.data.node_groups:
        if group.bl_idname == 'GeometryNodeTree' and group.get(OWNER_PROP
            ) == OWNER and group.get(ROLE_PROP) == role:
            return group
    group = bpy.data.node_groups.get(preferred_name)
    if group is not None and (group.bl_idname != 'GeometryNodeTree' or
        group.get(OWNER_PROP) != OWNER):
        group = bpy.data.node_groups.new(f'{preferred_name}__BPS',
            'GeometryNodeTree')
    elif group is None:
...
def _clear_interface(node_tree: bpy.types.NodeTree) ->None:
    """Function _clear_interface processes internal Blender data."""
...
def _clear_nodes(node_tree: bpy.types.NodeTree) ->None:
    """Function _clear_nodes processes internal Blender data."""
    node_tree.nodes.clear()


def _new_socket(node_tree: bpy.types.NodeTree, name: str, in_out: str,
    socket_type: str, *, default: (Any | None)=None, minimum: (float | None
    )=None, maximum: (float | None)=None) ->Any:
    """Function _new_socket processes internal Blender data."""
    socket = node_tree.interface.new_socket(name=name, in_out=in_out,
        socket_type=socket_type)
    if default is not None and hasattr(socket, 'default_value'):
        print("LOG (EXTERNAL API): socket.default_value = default")
        socket.default_value = default
    if minimum is not None and hasattr(socket, 'min_value'):
        print("LOG (EXTERNAL API): socket.min_value = minimum")
        socket.min_value = minimum
    if maximum is not None and hasattr(socket, 'max_value'):
        print("LOG (EXTERNAL API): socket.max_value = maximum")
        socket.max_value = maximum
    return socket


def _shade(color: Sequence[float], factor: float) ->tuple[float, float,
    float, float]:
    """Function _shade processes internal Blender data."""
    values = list(color[:3])
    return min(max(values[0] * factor, 0.0), 1.0), min(max(values[1] *
        factor, 0.0), 1.0), min(max(values[2] * factor, 0.0), 1.0), 1.0


def _set_node_location(node: bpy.types.Node, x: float, y: float
    ) ->bpy.types.Node:
    """Function _set_node_location processes internal Blender data."""
    node.location = x, y
    return node


def build_toon_material(base_color: Sequence[float]=(0.48, 0.07, 0.12, 1.0)
    ) ->bpy.types.Material:
    ...
    _clear_nodes(tree)
    output = _set_node_location(tree.nodes.new('ShaderNodeOutputMaterial'),
        860, 40)
    ...
    shader_to_rgb = _set_node_location(tree.nodes.new(
        'ShaderNodeShaderToRGB'), -160, 150)
    shader_to_rgb.name = 'BPS Eevee Shader to RGB'
    diffuse = _set_node_location(tree.nodes.new('ShaderNodeBsdfDiffuse'), -
        420, 150)
    diffuse.name = 'BPS Lighting Probe'
    diffuse.inputs['Color'].default_value = 1.0, 1.0, 1.0, 1.0
    diffuse.inputs['Roughness'].default_value = 0.25
    variation_ramp = _set_node_location(tree.nodes.new('ShaderNodeValToRGB'
        ), 100, -170)
    variation_ramp.name = 'BPS Restrained Material Variation'
    variation_ramp.color_ramp.elements[0].position = 0.28
    variation_ramp.color_ramp.elements[0].color = 0.8, 0.8, 0.8, 1.0
    ...
    texcoord = _set_node_location(tree.nodes.new('ShaderNodeTexCoord'), -
        420, -190)
...
def build_outline_material(outline_color: Sequence[float]=(0.005, 0.008,
    0.012, 1.0)) ->bpy.types.Material:
...
def build_outline_group(outline_material: bpy.types.Material, *, width:
    float=0.025, noise_scale: float=5.0, wobble_amount: float=0.0
    ) ->bpy.types.NodeTree:
    ...
    _new_socket(group, 'Width', 'INPUT', 'NodeSocketFloat', default=width,
        minimum=0.0, maximum=max(width * 20.0, 1.0))
    _new_socket(group, 'Noise Scale', 'INPUT', 'NodeSocketFloat', default=
        noise_scale, minimum=0.01, maximum=1000.0)
    _new_socket(group, 'Wobble Amount', 'INPUT', 'NodeSocketFloat', default
        =wobble_amount, minimum=0.0, maximum=max(width * 4.0, 0.25))
    _new_socket(group, 'Outline Material', 'INPUT', 'NodeSocketMaterial',
    ...
    extrude = _set_node_location(nodes.new('GeometryNodeExtrudeMesh'), -860,
        160)
    ...
    separate = _set_node_location(nodes.new('GeometryNodeSeparateGeometry'),
        -630, 160)
    separate.name = 'BPS Keep Extruded Top Shell'
    separate.domain = 'FACE'
    set_position = _set_node_location(nodes.new('GeometryNodeSetPosition'),
        -170, 170)
    ...
    set_material = _set_node_location(nodes.new('GeometryNodeSetMaterial'),
        280, 170)
    ...
    position = _set_node_location(nodes.new('GeometryNodeInputPosition'), -
        850, -210)
    position.name = 'BPS Spatial Noise Coordinates'
    normal = _set_node_location(nodes.new('GeometryNodeInputNormal'), -400,
        -310)
    ...
    center_noise = _set_node_location(nodes.new('ShaderNodeMapRange'), -400,
        -130)
    ...
    normal_scale = _set_node_location(nodes.new('ShaderNodeVectorMath'), 50,
        -110)
    ...
    links.new(group_input.outputs['Outline Material'], set_material.inputs[
        'Material'])
...
def _interface_input_identifiers(group: bpy.types.NodeTree) ->dict[str, str]:
    """Function _interface_input_identifiers processes internal Blender data."""
    identifiers: dict[str, str] = {}
    for item in group.interface.items_tree:
        if getattr(item, 'item_type', '') == 'SOCKET' and getattr(item,
            'in_out', '') == 'INPUT':
            identifiers[item.name] = item.identifier
    return identifiers


def _ensure_outline_modifier(obj: bpy.types.Object, group: bpy.types.
    NodeTree, *, outline_material: bpy.types.Material, width: float,
    noise_scale: float, wobble_amount: float) ->bpy.types.Modifier:
    """Function _ensure_outline_modifier processes internal Blender data."""
    modifier = next((candidate for candidate in obj.modifiers if candidate.
        type == 'NODES' and candidate.node_group is not None and candidate.
        node_group.get(OWNER_PROP) == OWNER and candidate.node_group.get(
        ROLE_PROP) == 'inverted_hull'), None)
    if modifier is None:
        modifier = obj.modifiers.new(OUTLINE_MODIFIER_NAME, 'NODES')
    modifier.node_group = group
    identifiers = _interface_input_identifiers(group)
    values = {'Enable': True, 'Width': float(width), 'Noise Scale': float(
        noise_scale), 'Wobble Amount': float(wobble_amount),
...
def _assign_material(obj: bpy.types.Object, material: bpy.types.Material, *,
    replace_material: bool) ->str:
    """Function _assign_material processes internal Blender data."""
    if replace_material:
...
def selected_mesh_objects() ->list[bpy.types.Object]:
    """Function selected_mesh_objects processes internal Blender data."""
    return [obj for obj in bpy.context.selected_objects if obj.type ==
        'MESH' and not obj.hide_get()]


def _set_eevee_engine() ->str:
    """Function _set_eevee_engine processes internal Blender data."""
...
def apply_to_objects(objects: Iterable[bpy.types.Object], *, width: float=
    0.025, noise_scale: float=5.0, wobble_amount: float=0.0, base_color:
    Sequence[float]=(0.48, 0.07, 0.12, 1.0), outline_color: Sequence[float]
    =(0.005, 0.008, 0.012, 1.0), replace_material: bool=False) ->dict[str, Any
    ]:
    ...
    outline_group = build_outline_group(outline_material, width=width,
        noise_scale=noise_scale, wobble_amount=wobble_amount)
    results = []
    for obj in mesh_objects:
        assignment = _assign_material(obj, toon_material, replace_material=
            replace_material)
        modifier = _ensure_outline_modifier(obj, outline_group,
            outline_material=outline_material, width=width, noise_scale=
            noise_scale, wobble_amount=wobble_amount)
        results.append({'object': obj.name, 'material_action': assignment,
            'outline_modifier': modifier.name})
    return {'schema_version': SCHEMA_VERSION, 'owner': OWNER,
        'render_engine': bpy.context.scene.render.engine, 'objects':
        results, 'toon_material': toon_material.name, 'outline_material':
        outline_material.name, 'outline_group': outline_group.name,
        'outline_width': width, 'noise_scale': noise_scale, 'wobble_amount':
        wobble_amount, 'replace_material': replace_material}
...
def main() ->int:
    ...
    parser.add_argument('--base-color', type=float, nargs=3, default=(0.48,
        0.07, 0.12))
    parser.add_argument('--outline-color', type=float, nargs=3, default=(
        0.005, 0.008, 0.012))
    ...
    if not objects:
        raise RuntimeError(
            'Select at least one visible mesh object before running this script'
            )
    report = apply_to_objects(objects, width=args.width, noise_scale=args.
        noise_scale, wobble_amount=args.wobble, base_color=(*args.
        base_color, 1.0), outline_color=(*args.outline_color, 1.0),
